...
class ExpressionEvaluator(object):
    def __init__(self, tables):
        ...
        self.parser = PyPikaParser({table._table_name: table for table in self._maybe_convert_pypika_tables(tables)})

    def _maybe_convert_pypika_tables(self, tables: List[Tuple]):
        ptables = [table if isinstance(table, Table) else Table(table[0], alias=table[1]) for table in tables]
        return ptables
